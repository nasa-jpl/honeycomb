...
function reloadPage() {
    ...
    } else if (window.HONEYCOMB_APP_CONFIG_PATH) {
        Config.load(window.HONEYCOMB_APP_CONFIG_PATH).then(config => {
            loadConfig(config, true);
        });
    } else if (window.HONEYCOMB_APP_CONFIG) {
        loadConfig(window.HONEYCOMB_APP_CONFIG, true);
    } else {
        const landingPage = (
            <MuiThemeProvider theme={appTheme}>
